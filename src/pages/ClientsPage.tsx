...
function ClientsPage() {
  ...
  const handleAddClient = async () => {
    ...
    if (newClientPhone && !validateSaudiNumber(newClientPhone)) { newErrors.phone = 'يجب أن يبدأ بـ 05 ويتكون من 10 أرقام'; hasError = true; }
    ...
    if (editingClient) {
        ...
        const newClient: Client = { id: Date.now(), name: newClientName, phone: newClientPhone ? `966${newClientPhone.substring(1)}` : '', whatsapp: newClientWhatsapp ? `966${newClientWhatsapp.substring(1)}` : '', createdAt: Date.now(), createdBy: currentUser?.officeName };
        if (currentUser) {
            const targetId = currentUser.role === 'employee' && currentUser.parentId ? currentUser.parentId : currentUser.id;
    ...
    setLoading(false);
    setNewClientName(''); setNewClientPhone(''); setNewClientWhatsapp('');
  ...
  const handleEditClient = (e: React.MouseEvent, client: Client) => {
      ...
      let phone = client.phone || ''; if(phone.startsWith('966')) phone = '0' + phone.substring(3); setNewClientPhone(phone);
    ...
    <div className="max-w-4xl mx-auto pb-20">
      ...
      <div className="flex gap-4 mb-6">
        <div className="relative flex-1">
            ...
            <Input placeholder="بحث عن عميل..." className="pr-10 bg-[#eef2f6] shadow-3d-inset border-none" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
        <Dialog open={open} onOpenChange={(val) => { if(val && !checkAddPermission() && !editingClient) return; setOpen(val); if(!val) { setEditingClient(null); setNewClientName(''); setNewClientPhone(''); setNewClientWhatsapp(''); } }}>
            <DialogTrigger asChild><button className="bg-blue-600 text-white px-6 rounded-xl font-bold shadow-3d hover:bg-blue-700 flex items-center gap-2"><Plus className="w-5 h-5" /> أضف عميل</button></DialogTrigger>
            <DialogContent className="bg-[#eef2f6] shadow-3d border-none" dir="rtl">
                ...
                <div className="space-y-4 py-4">
                    ...
                    {editingClient && (<button onClick={(e) => handleDeleteClient(e, editingClient.id)} className="w-full py-3 bg-red-100 text-red-600 rounded-xl font-bold shadow-sm hover:bg-red-200 flex items-center justify-center gap-2 mt-2"><Trash2 className="w-4 h-4" /> حذف العميل</button>)}
                </div>
            </DialogContent>
        </Dialog>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filteredClients.map(client => (
            <div key={client.id} onClick={() => handleClientClick(client)} className="bg-[#eef2f6] p-4 rounded-2xl shadow-3d hover:shadow-3d-hover cursor-pointer transition-all flex items-center justify-between gap-4 border border-white/50 relative group">
                <div className="flex items-center gap-4"><div className="w-12 h-12 bg-blue-100 rounded-full flex items-center justify-center text-blue-600 shadow-sm"><Users className="w-6 h-6" /></div><div><h3 className="font-bold text-gray-700 text-lg">{client.name}</h3><div className="flex gap-2 text-xs text-gray-400 mt-1">{client.phone && <span className="flex items-center gap-1" dir="ltr">+{client.phone} <Phone className="w-3 h-3"/></span>}</div></div></div>
                <div className="flex gap-2"><button onClick={(e) => handleEditClient(e, client)} className="w-8 h-8 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center shadow-sm hover:bg-blue-100"><Pencil className="w-4 h-4" /></button>{client.phone && (<button onClick={(e) => handlePhoneClick(e, client.phone)} className="w-8 h-8 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center shadow-3d hover:scale-110 transition-transform"><Phone className="w-4 h-4" /></button>)}{client.whatsapp && (<button onClick={(e) => handleWhatsAppClick(e, client.whatsapp)} className="w-8 h-8 rounded-full bg-green-100 text-green-600 flex items-center justify-center shadow-3d hover:scale-110 transition-transform"><MessageCircle className="w-4 h-4" /></button>)}</div>
            </div>
        ))}
      </div>
      <Dialog open={!!selectedClient} onOpenChange={(open) => !open && setSelectedClient(null)}>
        <DialogContent className="bg-[#eef2f6] shadow-3d border-none max-w-2xl" dir="rtl">
            <DialogHeader><DialogTitle className="text-xl font-bold flex items-center gap-2"><Users className="w-5 h-5 text-blue-600" /> معاملات العميل: {selectedClient?.name}</DialogTitle></DialogHeader>
            <div className="py-4 space-y-3 max-h-[50vh] overflow-y-auto">
                {clientTxs.length > 0 ? clientTxs.map(tx => (
                    <div key={tx.id} className="bg-white/50 p-3 rounded-xl flex justify-between items-center border border-white"><div className="flex items-center gap-3"><FileText className="w-4 h-4 text-gray-400" /><div><p className="font-bold text-sm text-gray-700">{tx.type}</p><p className="text-xs text-gray-500">{new Date(tx.createdAt).toLocaleDateString('ar-SA')}</p></div></div><div className="text-left"><p className="font-bold text-blue-600">{tx.clientPrice} ر.س</p><div className="text-[10px] font-bold">{tx.status === 'cancelled' ? (tx.clientRefunded ? <span className="text-gray-400">ملغاة (تم الاسترجاع)</span> : <span className="text-red-500">ملغاة (مستحق الاسترجاع)</span>) : tx.status === 'completed' ? (<span className="text-green-600">منجزة</span>) : (<span className="text-orange-500">قيد التنفيذ</span>)}</div></div></div>
            ...
            {clientTxs.some(t => t.status === 'cancelled' && !t.clientRefunded) && totalRefundDue > 0 && (
                <div className="mt-2 pt-4 border-t border-gray-200">
                    {refundStep === 'summary' && (
                        <div className="flex items-center justify-between bg-white p-4 rounded-xl shadow-3d-inset"><button onClick={() => setRefundStep('bank-select')} className="flex items-center gap-2 px-6 py-3 bg-red-600 text-white rounded-xl font-bold shadow-lg hover:bg-red-700 transition-all"><ArrowDownLeft className="w-4 h-4" /> تحويل للعميل (استرجاع)</button><div className="text-left"><p className="text-xs text-gray-500 font-bold mb-1">إجمالي المسترجع</p><p className="text-2xl font-black text-red-600">{totalRefundDue.toLocaleString()} ر.س</p></div></div>
                    )}
                    ...
                    {refundStep === 'success' && (
                        <div className="bg-green-50 border border-green-200 p-4 rounded-xl text-center space-y-4 animate-in zoom-in"><div className="flex flex-col items-center gap-2"><div className="w-12 h-12 bg-green-100 rounded-full flex items-center justify-center text-green-600"><CheckCircle2 className="w-6 h-6" /></div><h3 className="font-bold text-green-800">تم ارجاع المبلغ للعميل وحذف المعامله من السجل</h3></div><div className="grid grid-cols-2 gap-3"><button onClick={sendRefundWhatsApp} className="flex items-center justify-center gap-2 py-3 bg-green-600 text-white rounded-xl font-bold shadow-lg hover:bg-green-700"><Send className="w-4 h-4" /> إرسال المبلغ للعميل</button><button onClick={() => setSelectedClient(null)} className="flex items-center justify-center gap-2 py-3 bg-gray-200 text-gray-700 rounded-xl font-bold hover:bg-gray-300"><X className="w-4 h-4" /> خروج</button></div></div>
                    )}
                </div>
            )}
        </DialogContent>
